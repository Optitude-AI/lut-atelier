...
export default function CLGrid({ className = '' }: CLGridProps) {
  ...
  // Drag dead zone — prevents micro-drags during double-click from corrupting offsets
  ...
  const syncToStore = useCallback(() => {
    ...
    const storeNodes = ns.map((n) => ({
      ...
      // Luminance: ±15% max for flexible lightness control
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (isDraggingRef.current && dragInfoRef.current) {
        ...
        // Dead zone: don't apply drag until mouse moves at least 3px
