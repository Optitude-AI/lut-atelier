...
export default function Workspace() {
  ...
  const neutralProtection = settings.neutralProtection;
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          >
            <div className="w-[400px] h-full flex flex-col bg-zinc-950/50">
              ...
              {/* CL Axis Selector + Neutral Protection */}
              {activeGridType === 'cl' && (
                <div className="flex items-center gap-2 px-3 py-1.5 border-b border-zinc-800/60 bg-zinc-900/30 flex-shrink-0">
                  <span className="text-[9px] text-zinc-500 font-medium uppercase tracking-wider whitespace-nowrap">Axis</span>
                  <div className="flex items-center bg-zinc-900 rounded-md p-0.5 gap-px">
                    {(['all', 'red-cyan', 'green-magenta', 'blue-yellow'] as const).map((ax) => (
                      <button
                        key={ax}
                        onClick={() => setCLAxis(ax)}
                        className={
                          `h-5 px-1.5 text-[9px] font-medium rounded transition-colors ` +
                          (clAxis === ax
                            ? 'bg-zinc-700 text-zinc-200'
                            : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800')
                        }
                      >
                        {ax === 'all' ? 'All' : ax === 'red-cyan' ? 'R/Cy' : ax === 'green-magenta' ? 'G/Mg' : 'B/Ye'}
                      </button>
                    ))}
                  </div>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        onClick={() => updateSettings({ neutralProtection: !neutralProtection })}
                      ...
                      >
                        {neutralProtection ? <ShieldCheck className="w-3 h-3" /> : <ShieldOff className="w-3 h-3" />}
                        <span className="hidden sm:inline">Neutral Protect</span>
                      </button>
                    </TooltipTrigger>
                    <TooltipContent side="top" className="text-[10px]">Protect neutral/grey tones from color shifts</TooltipContent>
                  </Tooltip>
                </div>
              )}
